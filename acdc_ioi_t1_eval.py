...
def parse_args():
    ...
    parser.add_argument("--dataset-path", "-d", default="/scratch/network/mc3803/Edge-Pruning/data/datasets/ioi-t1-t1/")
    ...
    if args.out_json_path is None:
        args.out_json_path = f"results/ioi-t1-t1-sweep/ioi-t1-t{args.threshold}-graph.json"
    if args.out_pickle_path_final is None:
        args.out_pickle_path_final = f"results/ioi-t1-t1-sweep/ioi-t1-t{args.threshold}-graph.pkl"
    return args
...
import os
os.makedirs("results/ioi-t1-t1-sweep", exist_ok=True)
