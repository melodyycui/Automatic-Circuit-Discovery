...
DATASET=$EDGE_REPO/data/datasets/ioi
JSON_OUT=$EDGE_REPO/data/acdc_results/ioi-t${THRESHOLD}-graph.json
CKPT_OUT=$EDGE_REPO/data/acdc_checkpoints/ioi-t${THRESHOLD}
...
sbatch --partition=gpu --gres=gpu:1 --mem=32G --time=12:00:00 --constraint=gpu80 \
    --job-name=acdc_ioi \
    --output=$EDGE_REPO/joblog/acdc_ioi_t${THRESHOLD}_%j.out \
    --wrap="cd $EDGE_REPO && \
    module load anaconda3/2025.12 && \
    conda activate /scratch/network/mc3803/envs/acdc2 && \
    export TRANSFORMERS_OFFLINE=1 && \
    export HF_HOME=/scratch/network/mc3803/.cache/huggingface && \
    echo '[Step 1] Running ACDC on IOI...' && \
    python $ACDC_REPO/acdc_ioi_eval.py \
        --threshold $THRESHOLD \
        --dataset-path $DATASET \
        --max-train-examples 200 \
        --max-num-epochs 100000 \
        --out-json-path $JSON_OUT \
        --out-pickle-path-final $EDGE_REPO/data/acdc_results/ioi-t${THRESHOLD}-graph.pkl && \
    ...
    python $EDGE_REPO/src/eval/eval_acdc_ioi.py \
        --acdc-json-path $JSON_OUT \
        --data-path $DATASET"