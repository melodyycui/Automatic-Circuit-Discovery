...
DATASET=$EDGE_REPO/data/datasets/gp
JSON_OUT=$EDGE_REPO/data/acdc_results/gp-t${THRESHOLD}-graph.json
CKPT_OUT=$EDGE_REPO/data/acdc_checkpoints/gp-t${THRESHOLD}
...
sbatch --partition=gpu --gres=gpu:1 --mem=32G --time=12:00:00 --constraint=gpu80 \
    --job-name=acdc_gp \
    --output=$EDGE_REPO/joblog/acdc_gp_t${THRESHOLD}_%j.out \
    --wrap="cd $EDGE_REPO && \
    module load anaconda3/2025.12 && \
    conda activate /scratch/network/mc3803/envs/acdc2 && \
    export TRANSFORMERS_OFFLINE=1 && \
    export HF_HOME=/scratch/network/mc3803/.cache/huggingface && \
    echo '[Step 1] Running ACDC on GP...' && \
    python $ACDC_REPO/acdc_gp_eval.py \
        --threshold $THRESHOLD \
        --dataset-path $DATASET \
        --max-train-examples 150 \
        --max-num-epochs 100000 \
        --out-json-path $JSON_OUT \
        --out-pickle-path-final $EDGE_REPO/data/acdc_results/gp-t${THRESHOLD}-graph.pkl && \
    ...
    python $EDGE_REPO/src/eval/eval_acdc_gp.py \
        --acdc-json-path $JSON_OUT \
        --model-path $CKPT_OUT \
        --data-path $DATASET"